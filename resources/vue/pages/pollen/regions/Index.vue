<template>
    ...
    <PPage v-else full-width title="Pollen Regions">
    ...
    methods: {
        ...
        async getPollenRegions () {
            ...
            try {
                ...
                this.pollenRegions = data.pollenRegions?.data || [];
                this.pagination.hasNext = !!(data.pollenRegions?.next_page_url);
                this.pagination.hasPrevious = !!(data.pollenRegions?.prev_page_url);
                this.pagination.from = data.pollenRegions?.from;
                this.pagination.to = data.pollenRegions?.to;
                this.pagination.total = data.pollenRegions?.total;
            } catch (e) {
                this.$pToast.open({
        ...
        handleAdd() {
            this.$router.push({name: 'pollen.regions.create'});
        },
        handleEdit(id) {
            this.$router.push({name: 'pollen.regions.edit', params: {regionId: id}});
        },
        async handleDelete(item) {
            let isConfirmed = await this.$root.$confirm('Delete', `Are you sure want to delete <b>${item.name}</b> pollen region?`, {
                primaryAction: {
                    destructive: true,
                },
            });
            ...
            try {
                await axios.delete(`/app/pollen/regions/${item.id}`);
                await this.getPollenRegions();
                this.$pToast.open({
                    message: 'POLLEN Region removed successfully',
                });
            } catch (e) {
                this.$pToast.open({
                    message: e.message || 'Something went wrong',
