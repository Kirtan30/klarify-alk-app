...
export default {
    ...
    methods: {
        ...
        async getAllergyTests() {
            this.loading = true;
            try {
                let {data} = await axios.get('app/allergy-test-swedish', {
                    params: this.options
                });
                this.pagination.hasNext = !!(data.allergyTestsSwedish?.next_page_url);
                this.pagination.hasPrevious = !!(data.allergyTestsSwedish?.prev_page_url);
                this.pagination.from = data.allergyTestsSwedish?.from || 0;
                this.pagination.to = data.allergyTestsSwedish?.to || 0;
                this.pagination.total = data.allergyTestsSwedish?.total || 0;
                this.allergyTests = data.allergyTestsSwedish.data || [];
            } catch (exception) {
                this.$pToast.open({
        ...
        async generatePdf(uuid) {
            try {
                let res = await axios.get(`app/allergy-test-swedish/${uuid}/download`, {
                    responseType: 'blob'
                })
        ...
        async handleAggregateResults() {
            ...
            try {
                let { data } = await axios.get(`app/allergy-test-swedish/exportcsv`, {
