...
export default {
    ...
    methods: {
        ...
        async handleSaveStaticContent() {
            ...
            try {
                ...
                let { data } = this.form.id ?
                    await axios.put(`/app/fad/static-contents/${this.form.id}`, parameters) :
                    await axios.post('/app/fad/static-contents', parameters);
                this.manageTempForm();
                this.$pToast.open(data.message || 'Static Content saved successfully');
                await this.$router.push({name: 'fad.static-contents'});
            } catch ({response}) {
                this.$pToast.open({
                    error: true,
        ...
        async getStaticContent(staticContentId) {
            try {
                let {data} = await axios.get(`/app/fad/static-contents/${staticContentId}`);
                this.form = data?.fadStaticContent || {};
                let variables = [];
