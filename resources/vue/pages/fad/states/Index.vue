<template>
    ...
    <PPage v-else full-width title="FAD States">
...
export default {
    ...
    methods: {
        ...
        async getFadStates () {
            this.loading = true;
            try {
                let { data } = await axios.get('/app/fad/states', {
                    params: this.options
                })
                ...
                this.pagination.hasNext = !!(data.fadStates?.next_page_url);
                this.pagination.hasPrevious = !!(data.fadStates?.prev_page_url);
                this.pagination.from = data.fadStates?.from;
                this.pagination.to = data.fadStates?.to;
                this.pagination.total = data.fadStates?.total;
            } catch (e) {
                this.$pToast.open({
        ...
        // handleAdd() {
        //     this.$router.push({name: 'fad.states.create'});
        // },
        handleEdit(id) {
            this.$router.push({name: 'fad.states.edit', params: {stateId: id}});
        },
        // async handleDelete(item) {
        //     let isConfirmed = await this.$root.$confirm('Delete', `Are you sure want to delete <b>${item.name}</b> fad state?`, {
        //         primaryAction: {
        //             destructive: true,
        //         },
        //     });
        ...
        //     }
        //     try {
        //         await axios.delete(`/app/fad/states/${item.id}`);
        //         await this.getFadStates();
        //         this.$pToast.open({
        //             message: 'FAD State removed successfully',
        //         });
        //     } catch (e) {
        //         this.$pToast.open({
        //             message: e.message || 'Something went wrong',
